...
with open('D:\loan approval\models\xgb_cv_final.pkl', 'rb') as f:
    clf_individual = pickle.load(f) 
#load models at top of app to load into memory only one time
with open('D:\loan approval\models\xgb_cv_compact.pkl', 'rb') as f:
    clf_joint = pickle.load(f)

# with open('C:/Users/vinay/Downloads/loan-default-prediction-master/loan-default-prediction-master/models/knn_regression.pkl', 'rb') as f:
#     knn = pickle.load(f)    
# ss = StandardScaler()
#feature space
df_train_jl_scale = pd.read_csv('D:\loan approval\data\df_train_jl_scale.csv')
#load APR table
df_fico_apr = pd.read_csv('D:\loan approval\data\df_macro_mean.csv')

df_macro_mean  = pd.read_csv('D:\loan approval\data\df_macro_mean.csv', index_col=0, dtype=np.float64)

df_macro_std = pd.read_csv('D:\loan approval\data\df_macro_std.csv', index_col=0, dtype=np.float64)
